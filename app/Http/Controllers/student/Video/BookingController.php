<?php

namespace App\Http\Controllers\student\Video;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Models\VideoCourse\VideoCourse;
use App\Models\VideoCourse\VideoBooking;
use App\Models\MerchantBooking;

class BookingController extends Controller
{
    public function __construct()
    {
        $this->middleware('auth');
    }

    public function index()
    {
        $bookings = auth()->user()->video_bookings;
        return view('student.videocourse.booking.index',compact('bookings'));
    }

    public function create()
    {
        $courses = VideoCourse::where('status','=','Active')->get();
        return view('student.videocourse.booking.create',compact('courses'));
    }

    public function store(Request $request)
    {
        // dd($request->all());
        $request->validate([
            'course'=>'required|numeric|min:1',
            'remarks'=>'string|nullable',
        ]);
        $search=VideoBooking::where([
            ['course_id','=',$request->course],
            ['user_id','=',auth()->user()->id],
        ])->count();
            
        if($search){
            return back()->withInput()->with('alreadybooked', 'You Have Already Booked This Video Course!');
        }

        $booking= VideoBooking::create([
            'user_id'=>auth()->user()->id,
            'course_id'=>$request->course,
            'user_name'=>auth()->user()->name,
            'status'=>'Unverified',
            'updatedBy'=>auth()->user()->name,
            'remarks'=>$request->remarks,
        ]);

        return redirect('/student/video-course/'.$booking->id.'/edit');
    }

    public function edit(VideoBooking $booking)
    {
        return view('student.videocourse.booking.edit',compact('booking'));
    }

    public function update(Request $request, VideoBooking $booking)
    {
        // dd($request->all(),$booking);
        $request->validate([
            "verificationMode" => "string|required|min:1",
            "paymentAmount" => "numeric|required",
            "verificationDocument" => "image|required",
        ]);
        $img = request('verificationDocument')->store('uploads','public');
        $booking->update([
            'verificationMode' => $request->verificationMode,
            'paymentAmount' => $request->paymentAmount,
            'verificationDocument' => $img,
            'status' => 'Processing',
        ]);

        return redirect('/student/video-course');
    }

    public function destroy(VideoBooking $booking)
    {
        $booking->delete();
        return redirect('/student/video-course');
    }

    public function paymentFailed(VideoBooking $booking)
    {
        return redirect("/student/video-course/$booking->id/edit")->with('error_message','Transaction Failed. Try Again Later.');
    }

    public function esewaSuccess(VideoBooking $booking, Request $request)
    {
        if(isset($request->oid) && isset($request->amt) && isset($request->refId))
        {
            $url = "https://esewa.com.np/epay/transrec";
            $data =[
                'amt'=> ($booking->course->fee - $booking->course->discount),
                'rid'=> $request->refId,
                'pid'=> $request->oid,
                'scd'=> 'NP-ES-ODADEPL'
            ];
            
            $curl = curl_init($url);
            curl_setopt($curl, CURLOPT_POST, true);
            curl_setopt($curl, CURLOPT_POSTFIELDS, $data);
            curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
            $response = curl_exec($curl);
            curl_close($curl);
            // dd($response);
            $response_code =trim($this->get_xml_node_value('response_code',$response));
            if($response_code=='Success')
            {
                $booking->update([
                    'status'=>'Verified',
                    'verificationMode'=>'Esewa',
                    'paymentAmount'=>$data['amt'],
                    'remarks'=>'Booked by Student with Direct Esewa Payment',
                    'updatedBy'=>auth()->user()->name,
                ]);
                MerchantBooking::create([
                    'type' => 'video',
                    'title' => $booking->course->name ?? '',
                    'merchant' => 'esewa',
                    'booking_id' => $booking->id,
                ]);
                return redirect('/student/video-course')->with('success_message','Transction Completed Succesfully.');
            }
            
        }

        return redirect("/student/video-course/$booking->id/edit")->with('error_message','Transaction Failed. Try Again Later.');
    }

    public function khaltiSuccess(VideoBooking $booking, Request $request)
    {
        $args = http_build_query(array(
            'token' => $request->token,
            'amount'  => ($booking->course->fee - $booking->course->discount) * 100
        ));
        
        $url = "https://khalti.com/api/v2/payment/verify/";
        
        # Make the call using API.
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS,$args);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        
        $headers = ['Authorization: Key test_secret_key_ac3b9ea5852c45d597d141b28d2c7c44'];
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        
        // Response
        $response = curl_exec($ch);
        $status_code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
        curl_close($ch);
        
        if($status_code == 200)
        {
            $booking->update([
                'status'=>'Verified',
                'verificationMode'=>'Khalti',
                'paymentAmount'=>($booking->course->fee - $booking->course->discount),
                'remarks'=>'Booked by Student with Direct Khalti Payment',
                'updatedBy'=>auth()->user()->name,
            ]);
            MerchantBooking::create([
                'type' => 'video',
                'title' => $booking->course->name ?? '',
                'merchant' => 'khalti',
                'booking_id' => $booking->id,
            ]);
            return response()->json([
                'success' => 1,
                'redirecto' => url('/student/video-course')
            ], 200);
        }
        else
        {
            return response()->json([
                'error' => 1,
                'message' => 'Payment Failed. Please try again later.'
            ]);
        }
        
    }

    public function get_xml_node_value($node, $xml)
    {
        if($xml==false)
        {
            return false;
        }

        $found=preg_match('#<'.$node.'(?:\s+[^>]+)?>(.*?)'.'</'.$node.'>#s',$xml,$matches);

        if($found!=false)
        {
            return $matches[1];
        }

        return false;
    }
}
