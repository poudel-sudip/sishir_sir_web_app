    ...
    <div class="content-wrapper">
        ...
        <div class="row justify-content-center">
            <div class="col-md-8 grid-margin stretch-card">
                <div class="card">
                    <div class="card-header">Add Course Bookings</div>
                    <div class="card-body">
                        <form method="POST" id="verifyCourseForm" action="/vendor/bookings" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="form-group row">
                                <label for="coursePrice" class="col-md-4 col-form-label">{{ __('Course Price') }}</label>

                                <div class="col-md-8">
                                    <input id="coursePrice" type="text" class="form-control @error('coursePrice') is-invalid @enderror" name="coursePrice" value="{{ $batch->fee - $batch->discount }}" readonly >
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input id="myPrice" type="text" class="form-control @error('myPrice') is-invalid @enderror" name="myPrice" value="{{ floor(($batch->fee - $batch->discount) * (auth()->user()->vendor->vendor_discount/100)) }}" readonly >
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input id="paymentAmount" type="text" class="form-control @error('paymentAmount') is-invalid @enderror" name="paymentAmount" value="{{ old('paymentAmount') ?? (floor(($batch->fee - $batch->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count)) }}" >
                                    <label class="">Out of Rs. {{ floor(($batch->fee - $batch->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count) }} </label>
...
<script>
    ...
    function pay_esewa(userids)
    {
        ...
        var initurl='{{url("vendor/bookings/batch/$batch->id/users")}}';
        var params= {
          amt: {{ floor(($batch->fee - $batch->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count) }},
          psc: 0,
          pdc: 0,
          txAmt: 0,
          tAmt: {{ floor(($batch->fee - $batch->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count)}},
          pid: "{{time()}}",
          scd: "NP-ES-ODADEPL",
