...
@section('content')
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">Edit Provience User</h3>
        ...
        <div class="row justify-content-center">
            <div class="col-md-8 grid-margin stretch-card">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="/vendor/provience-users/{{$user->id}}" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input id="name" type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') ?? $user->name }}" required readonly>
                            ...
                            <div class="form-group row">
                                <label for="interests" class="col-md-4 col-form-label">{{ __('Select Interested Courses') }}</label>
                                ...
                                <div class="col-md-8">
                                    ...
                                    <div class="interest-form-group col-12 row">
                                        <select class="form-control col-9" id="courses">

                                            @foreach($courses as $course)
                                                <option value="{{$course->name}}">{{$course->name}}</option>
                                            @endforeach
                                        </select>
                                        <button class="ml-3 btn btn-info btn-sm my-3" onclick="javascript:courseadd()" type="button" >+</button>
                                    </div>
                                    @error('interests')
                                    <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
