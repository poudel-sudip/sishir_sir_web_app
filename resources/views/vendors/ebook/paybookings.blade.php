...
@section('content')
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title">E-Book Booking</h3>
        ...
        <div class="row justify-content-center">
            <div class="col-md-8 grid-margin stretch-card">
                <div class="card">
                    <div class="card-header">Add E-Book Bookings</div>
                    <div class="card-body">
                        <form method="POST" id="verifyCourseForm" action="/vendor/ebook-booking" enctype="multipart/form-data">
                            @csrf

                            <div class="form-group row">
                                <label for="book" class="col-md-4 col-form-label">{{ __('E-Book Name') }}</label>
                                <div class="col-md-8">
                                    <select name="book" id="book" class="form-control @error('book') is-invalid @enderror" value="{{ old('book') }}" required readonly>
                            ...
                            <div class="form-group row">
                                <label for="bookPrice" class="col-md-4 col-form-label">{{ __('E-Book Price') }}</label>

                                <div class="col-md-8">
                                    <input id="bookPrice" type="text" class="form-control @error('bookPrice') is-invalid @enderror" name="bookPrice" value="{{ $book->price - $book->discount }}" readonly >
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input id="myPrice" type="text" class="form-control @error('myPrice') is-invalid @enderror" name="myPrice" value="{{ floor(($book->price - $book->discount) * (auth()->user()->vendor->vendor_discount/100)) }}" readonly >
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input id="paymentAmount" type="text" class="form-control @error('paymentAmount') is-invalid @enderror" name="paymentAmount" value="{{ old('paymentAmount') ?? (floor(($book->price - $book->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count)) }}" >
                                    <label class="">Out of Rs. {{ floor(($book->price - $book->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count) }} </label>
...
<script>
    ...
    function pay_esewa(userids)
    {
        ...
        var initurl='{{url("vendor/ebook-booking/book/$book->id/users")}}';
        var params= {
          amt: {{ floor(($book->fee - $book->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count) }},
          psc: 0,
          pdc: 0,
          txAmt: 0,
          tAmt: {{ floor(($book->fee - $book->discount) * (auth()->user()->vendor->vendor_discount/100) * $user_count)}},
          pid: "{{time()}}",
          scd: "NP-ES-ODADEPL",
