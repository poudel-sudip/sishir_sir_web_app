...
@section('content')
        ...
        <div class="row">
            <div class="col-md-8">
                <div class="card student_verify_card">
                    <div class="card-header">{{ __('Booking ID: ') }} {{$booking->id}} | {{$booking->course->name ?? ''}}</div>

                    <div class="card-body enroll_form">
                        <form id="verifyCourseForm" method="POST" action="/student/video-course/{{$booking->id}}" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-8">
                                    <input id="course" type="text" class="form-control @error('course') is-invalid @enderror" name="course" value="{{ old('course') ?? ($booking->course->name.' @ Rs.'. ($booking->course->fee - $booking->course->discount)) }}" readonly>
                            ...
                            <div id="manualForm" class="d-none">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-8">
                                        <input id="paymentAmount" type="text" class="form-control @error('paymentAmount') is-invalid @enderror" name="paymentAmount" value="{{ old('paymentAmount') ?? $booking->paymentAmount ?? ($booking->course->fee - $booking->course->discount) }}" >
                            ...
                            <div class="form-group row mb-0">
                                <div class="col-md-6 offset-md-4">
                                    ...
                                    <a href="{{ url('/student/video-course') }}" class="btn btn-secondary">Verify Later</a>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    <script>
      ...
      function pay_esewa()
      {
        ...
        var params= {
            amt: {{ $booking->course->fee - $booking->course->discount }},
            psc: 0,
            pdc: 0,
            txAmt: 0,
            tAmt: {{$booking->course->fee - $booking->course->discount}},
            pid: "{{$booking->id.'-'.time()}}",
            scd: "NP-ES-ODADEPL",
            su: '{{url("student/video-course/$booking->id/esewaSuccess")}}',
            fu: '{{url("student/video-course/$booking->id/payment-failed")}}'
        };
    ...
    <script>
        function pay_khalti()
        {
            var config = {
                ...
                "productName": "{{$booking->course->name}}",
                "productUrl": "{{url('student/video-course')}}",
                "paymentPreference": [
                    "KHALTI",
                    "EBANKING",
                    "MOBILE_BANKING",
                ...
                "eventHandler": {
                    onSuccess (payload) {
                        ...
                        if(payload.idx)
                        {
                            ...
                            $.ajax({
                                method: 'POST',
                                url: '{{url("student/video-course/$booking->id/khaltiSuccess")}}',
                                data: payload,
                                ...
                                success: function(response) {
                                    ...
                                    {
                                        checkout.hide();
                                        window.location = '{{url("student/video-course/$booking->id/payment-failed")}}';
                                    }
                                },
            ...
            var amt = {{ $booking->course->fee - $booking->course->discount}};
