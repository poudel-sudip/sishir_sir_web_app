    ...
    <div class="student-content-wrapper student-enroll-section">
        <div class="row mb-2">
            <div class="col-md-12 text-end">
                <a class="student-enroll-btn" href="{{ url('/student/ebook/enroll') }}">Book E-Book</a>
            </div>
        </div>
        <div class="row">
            @foreach ($bookings as $booking)
                <div class="col-md-4 student-video-card mb-3">
                    <div class="vid-card-img-container">
                        ...
                        <a href="/student/ebook/{{$booking->id}}/chapters">
                            <img src="/storage/{{$booking->book->thumbnail}}" alt="" class="w-100"></a>
                        @else
                    ...
                    <div class="student-vid-dec">
                        ...
                        <div class="student-vid-status">
                            ...
                            <div class="text-end">
                                @if($booking->status!="Verified")
                                    <a href="/student/ebook/{{$booking->id}}/edit" class="btn btn-warning btn-sm">Verify</a> 
                                    <form id="delete-form-{{$booking->id}}" action="/student/ebook/{{$booking->id}}" method="POST" style="display: inline">
                                        @csrf
                                        @method('DELETE')
                                        <a href="javascript:{}" onclick="javascript:deleteData({{$booking->id}});" class="btn btn-danger btn-sm">Delete</a>
                                    </form>
                                {{-- @else
                                    <a href="/student/ebook/{{$booking->id}}/chapters" class="btn btn-primary btn-sm mb-1 ">Chapters</a>  --}}
                                @endif
                            </div>
                        </div>
                    </div>
