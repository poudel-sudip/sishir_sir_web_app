...
@section('content')
    <div class="content-wrapper">
        <div class="page-header">
            <h3 class="page-title"> Daily Question Comments</h3>
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="custon-table-header">
                            <h4 class="card-title"> Daily Question Comments</h4>
                        </div>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuOutlineButton1">
                                                    <form id="publish-form-{{$comment->id}}" action="/admin/daily-mcq-questions/{{$question->id}}/comments/{{$comment->id}}/Published" method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        <a href="javascript:{}" onclick="javascript:publishData({{$comment->id}});" class="text-primary dropdown-item">Publish</a>
                                                    </form>
                                                    <form id="unpublish-form-{{$comment->id}}" action="/admin/daily-mcq-questions/{{$question->id}}/comments/{{$comment->id}}/Unpublished" method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <a href="javascript:{}" onclick="javascript:unPublish({{$comment->id}});" class="text-warning dropdown-item">UnPublish</a>
                                                    </form>
                                                    <form id="delete-form-{{$comment->id}}" action="/admin/daily-mcq-questions/{{$question->id}}/comments/{{$comment->id}}/delete" method="POST">
                                                        @csrf
                                                        @method('DELETE')
